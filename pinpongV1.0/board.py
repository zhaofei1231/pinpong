...
class Pin:
  ...
  def __init__(self, board=None, pin=None, mode=None):
    ...
    self.mode = mode
    self.pin, self.apin = board.res["get_pin"](pin)
    if self.pin not in board.res["pin"]["dpin"]:
      raise ValueError("dpin error", self.pin) 
    if self.apin not in board.res["pin"]["apin"]:
      raise ValueError("apin error", self.apin) 
...
class DuinoPWM:
  def __init__(self, board, pin_obj):
    ...
    self.freq_value = 100
    self.duty_value = 50
    if pin_obj.pin not in board.res["pwm"]["pwmpin"]:
      raise ValueError("pin not support pwm", pin_obj.pin)
